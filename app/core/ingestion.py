...
def get_embeddings():
    """
    Creates and returns the embeddings model.

    Why a function instead of a module-level variable?
    Because if we create OpenAIEmbeddings at import time,
    it tries to read OPENAI_API_KEY immediately — before
    load_dotenv() has run. A function defers creation until
    the function is actually called, by which time the key is loaded.
    """
    return OpenAIEmbeddings(
        model=settings.openai_embedding_model,
        api_key=settings.openai_api_key  # pass explicitly — no os.environ needed
    )
...
def build_vectorstore(chunks: list) -> Chroma:
    ...
    vectorstore = Chroma.from_documents(
        documents=chunks,
        embedding=get_embeddings(),  # ← call function, not module-level variable
...
def load_existing_vectorstore() -> Chroma:
    ...
    return Chroma(
        persist_directory=settings.chroma_path,
        embedding_function=get_embeddings()  # ← call function
    )


def ingest(pdf_path: str) -> Chroma:
    """
    Idempotent ingestion — skips duplicates automatically.
    Creates a new vector store if none exists.
    Adds to existing vector store if the document is new.
    Skips entirely if the document is already stored.
    """
    file_hash = get_file_hash(pdf_path)
    file_name = Path(pdf_path).name

    # Case 1: No vector store yet — create from scratch
    if not Path(settings.chroma_path).exists():
        print("Creating new vector store...")
        chunks = load_and_split(pdf_path)
        return build_vectorstore(chunks)

    # Case 2: Vector store exists — check for duplicate
    vectorstore = Chroma(
        persist_directory=settings.chroma_path,
    ...
    # Case 3: New document — add to existing store
